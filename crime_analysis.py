...
if not os.path.exists(EXCEL_FILE):
    raise FileNotFoundError(f"{EXCEL_FILE} not found. Please download it from https://africa.ocindex.net/downloads")
...
crime_df['Total_Crime_Index'] = crime_df[[
    'Human_Trafficking', 'Arms_Trafficking', 'Flora_Crimes',
...
plt.title("Confusion Matrix: Predicted Crime Level")
plt.tight_layout()
plt.savefig("images/crime_level_confusion_matrix.png", bbox_inches="tight", dpi=300)
plt.show()
...
try:
    ...
    if len(yearly_data) >= 2:  # Need at least 2 data points
        ...
        time_series_df = time_series_df.set_index(pd.to_datetime(time_series_df['Year'], format='%Y'))
        ...
        available_metrics = [m for m in metrics_to_forecast if m in time_series_df.columns]
        ...
        def make_forecast(series_name):
            try:
                ...
                print(f"ARIMA failed for {series_name}, using linear regression: {str(e)}")
        ...
        for metric in available_metrics:
            plt.plot(time_series_df.index, time_series_df[metric],
                     'o-', linewidth=2, markersize=8,
                     label=f'Historical {metric.replace("Mean_", "")}')

        # Plot forecasts
        for metric in available_metrics:
            plt.plot(pd.date_range('2024', periods=4, freq='YE'),
                     forecasts[metric], 'x--', linewidth=2, markersize=10,
                     label=f'Forecast {metric.replace("Mean_", "")}')

        plt.title("African Regional Crime and Resilience Trends (2024-2027 Forecast)", pad=20)
        plt.xlabel("Year", labelpad=10)
        plt.ylabel("Score/Index Value", labelpad=10)
        plt.legend(bbox_to_anchor=(1.05, 1), frameon=True)
        plt.grid(True, alpha=0.3)
        plt.tight_layout()

        # Save high-quality version
        plt.savefig("images/comprehensive_regional_forecast.png",
                    bbox_inches="tight", dpi=300, transparent=False)
        plt.show()

        # Additional: Create interactive plot
        fig = px.line(title="African Regional Crime Trends Forecast")
        ...
        for metric in available_metrics:
            ...
            # Historical data
            fig.add_scatter(x=time_series_df.index, y=time_series_df[metric],
                            mode='lines+markers', name=f'Historical {clean_name}')

            # Forecast data
            fig.add_scatter(x=pd.date_range('2024', periods=4, freq='YE'),
                            y=forecasts[metric], mode='lines+markers',
                            name=f'Forecast {clean_name}', line=dict(dash='dot'))
        ...
        fig.write_html("images/interactive_regional_forecast.html")
        webbrowser.open('file://' + os.path.abspath('images/interactive_regional_forecast.html'))
...
sns.barplot(
    x='Criminality_Score',
    y='Country',
    data=crime_df_sorted,
    hue='Resilience_Bin',
    dodge=False,
    palette='viridis'
)

plt.title("African Countries by Organized Crime Index")
plt.xlabel("Criminality Score (0-10)")
plt.ylabel("Country")
plt.legend(title='Resilience Score', bbox_to_anchor=(1.05, 1))
plt.tight_layout()
plt.savefig("images/crime_index_countries.png", bbox_inches="tight", dpi=300)
plt.show()
...
plt.title("Crime vs Resilience Across Africa")
plt.grid(True)
plt.tight_layout()
plt.savefig("images/crime_vs_resilience.png", bbox_inches="tight", dpi=300)
plt.show()

# 3. Drug Trafficking Patterns Across Africa
...
drug_df['Drug Type'] = drug_df['Drug Type'].str.replace('_Trade', '').str.replace('_', ' ')
...
plt.title("Drug Trafficking Patterns Across Africa")
plt.xticks(rotation=45)
plt.tight_layout()
plt.savefig("images/drug_trafficking.png", bbox_inches="tight", dpi=300)
plt.show()

# 4. Feature Importance (for the Random Forest model)
plt.figure(figsize=(10, 6))
feat_importances = pd.Series([0.4, 0.3, 0.3], index=features)  # Example feature importances
feat_importances.nlargest(10).plot(kind='barh')
plt.title("Feature Importance for Crime Level Prediction")
plt.tight_layout()
...
fig.write_html("images/interactive_3d_plot.html")

# Open the interactive plot
html_path = os.path.abspath('images/interactive_3d_plot.html')
webbrowser.open('file://' + html_path)

# 6. Distribution of Criminality Score
plt.figure(figsize=(10, 6))
sns.histplot(crime_df['Criminality_Score'], kde=True, color='orange')
plt.title("Distribution of Criminality Score")
plt.tight_layout()
plt.savefig("images/criminality_score_distribution.png", bbox_inches="tight", dpi=300)
plt.show()

# 7. Resilience Score by Crime Level
plt.figure(figsize=(10, 6))
sns.boxplot(x='Crime_Level', y='Resilience_Score', data=crime_df, hue='Crime_Level', palette='coolwarm', legend=False)
plt.title("Resilience Score by Crime Level")
plt.tight_layout()
plt.savefig("images/resilience_score_by_crime_level.png", bbox_inches="tight", dpi=300)
plt.show()

# 8. Country vs Crime Index (Top 10 Countries)
top_10_countries = crime_df.sort_values(by="Total_Crime_Index", ascending=False).head(10)
...
plt.title("Top 10 African Countries by Crime Index")
plt.tight_layout()
plt.savefig("images/top_10_countries_crime_index.png", bbox_inches="tight", dpi=300)
plt.show()

# 9. Correlation Heatmap of Crime Indices
corr = crime_df[['Total_Crime_Index', 'Total_Drug_Trade', 'Resilience_Score']].corr()

plt.figure(figsize=(8, 6))
sns.heatmap(corr, annot=True, cmap='coolwarm', fmt='.2f')
plt.title("Correlation Heatmap of Crime Indices")
plt.tight_layout()
plt.savefig("images/correlation_heatmap.png", bbox_inches="tight", dpi=300)
plt.show()

# 10. Pairplot of Features
sns.pairplot(crime_df[features + ['Crime_Level']], hue='Crime_Level', palette='coolwarm')
plt.title("Pairplot of Features and Crime Level")
plt.tight_layout()
plt.savefig("images/pairplot_features.png", bbox_inches="tight", dpi=300)
plt.show()
